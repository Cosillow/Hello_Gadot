...
func _handle_collision(collision: KinematicCollision2D, delta: float) -> void:
	...
	# TODO: theres still the problem of multiple shape contacts... which I could actually just handle from here by checking for more intersections with the shapes
	...
	# TODO: do not resolve if velocities are separating along normal: 
				  ...
				  #if(velAlongNormal > 0)
					#return;
	var shape: CollisionShape2D = collision.get_local_shape() # NOTE: returns an object...
	...
	var contact_on_segment: Vector2 = Geometry2D.get_closest_point_to_segment(collision_point, shape.global_position + shape.shape.a, shape.global_position + shape.shape.b)
	...
	if collider is RigidBody2D:
		...
		var segment_start: Vector2 = shape.global_position + shape.shape.a
		var segment_end: Vector2 = shape.global_position + shape.shape.b
